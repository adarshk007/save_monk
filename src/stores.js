...
api_s.open("GET", "stores.json", true);
  	...
  	if (api_s.status >= 200 && api_s.status < 400) 
  		{	    var newNode = document.createElement('li');
            ...
            newNode.innerHTML = "<div class='col col-1' data-label='icon'><img src='"+(beta.Logo)+"' class='image_op'/></div>"+
                          "<div class='col col-2' data-label='Company'>"+(beta.Company)+"</div>"+
		    	                "<div class='col col-3' data-label='Location'>"+(beta.Location)+"</div>"+
                          ...
                          "<div class='col col-7' data-label='link'><button class='btn btn-danger' onclick='window.location.href='"+(beta.URL)+"'; return false;'>URL</button></div>"                          
//                           "<p>"+(beta.Item)+"</p>"+
...
//                   "</div>"
            document.getElementById('stores').appendChild(newNode);    
